...
function createDocViewer(holderid,src){
	if(!holderid)return false;
	
	$('#'+holderid).html('');
	Global_holderid=holderid;
	var fpViewer = new FlexPaperViewer(	
		'/App_Addins/flexpaper/FlexPaperViewer',
		holderid, 
		{
			config :{
				SwfFile : src,
...
window.FlexPaperViewer = window.$f = function() {
	...
	window.FlexPaperViewer_Instance = flashembed(arguments[1], {
			src: arguments[0]+".swf",
			version: [10, 0],
			expressInstall: "/App_Addins/flexpaper/expressinstall.swf"
		},{
			...
			key : config.key,
			onDocumentLoaded:function(data){
				alert(data);
			}
	});
};
