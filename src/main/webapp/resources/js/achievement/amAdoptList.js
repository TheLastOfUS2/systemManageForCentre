	...
	//调用后台方法
	function queryAction(adoptDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'adopt/adoptList?adoptDel='+adoptDel+"&getName="+getName+"&getValue="+getValue+queryParam + urls,
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
		         checkbox:true,
                ...
                formatter : function(value,row,index){  
     				 if(value!=null && value!=''){
     				   if(value=='0'){return '采纳'} else if(value=='1'){return '批示'}
     				   else if(value=='2'){return '刊发'}
     				 } else{return '--'}
               }
            }, {
                field : 'adoptSubmitTime',
                title : '提交时间',
                width : 80,
                sortable :true,
                formatter : function(value,row,index){  
	...
	function add(){
    	...
    	dlgCenter();
    	//点击保存按钮后跳转路径
    	url = "adopt/saveOrUpAdopt?collaboratorType=0" + urls;
	}
	...
	function edit(){
        ...
        if (row){
        	num=1;
        	$.ajax({
				url:"adopt/adoptView?adoptId="+row.adoptId + urls,
				type:"post",
				success:function (data){
					if(data!=null){
						$('#fm').form('load', data); 
						//将列表中的类型放到新增信息form中
						$('#adoptId').val(row.adoptId);
						$('#baseInfoPath').val(1);
        	...
        	//$('#fm').form('load', 'adopt/adoptView?path=editAdopt&adoptId='+row.adoptId); 
        }else{ 
	...
	function del(){
		...
		if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].adoptId;  
                 } else {  
                     temID = selRow[i].adoptId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(adoptDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('adopt/deleteAdopt?adoptDel='+adoptDel + urls,{adoptId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('adopt/deleteAdopt?adoptDel=0' + urls,{adoptId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		var url = "ReportServer?reportlet=adopt.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("采纳批示");
		window.open(url);
	}
	...
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="am_adopt"
		$("#relevanceId").val(row.adoptId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.adoptId);
	}
	
	function uploadList(tableName,adoptId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+adoptId+"&tableName="+tableName + urls,
			type:"post",
			success:function (data){
				$("#dlgUpload").dialog({  
   					//modal:true,遮罩层
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=adopt/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	function textHtml(data){
		//报告名称
		$("#adoptName").val(data.adoptName);
		//支持课题
 		$("#adoptSupportTopic").val(data.adoptSupportTopic);
 		//作者
 		$("#baseInfoId").val(data.baseInfoId);
 		//成果形式
 		$("#adoptWay").val(data.adoptWay);
 		//提交时间
 		$("#adoptSubmitTime").val(data.adoptSubmitTime);
 		//采纳刊物
 		$("#adoptPublication").val(data.adoptPublication);
 		//采纳时间
 		$("#adoptTime").val(data.adoptTime);
 		//采纳单位
 		$("#adoptCompany").val(data.adoptCompany);
 		//批示
 		$("#adoptEvaluate").val(data.adoptEvaluate);
 		//备注
 		$("#adoptRemarks").val(data.adoptRemarks);
	}
