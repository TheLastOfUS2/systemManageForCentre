	...
	$(document).ready(function() { 
		...
		// 当手动填写承担人时,清空id
		$("input",$("#entrustUndertaker").next("span")).keydown(function(){
			$("#entrustUndertakerId").val("");
		});
		//查询列表信息
		queryAction(0,'','');
		//改变数据为删除状态
	...
	//调用后台方法
	function queryAction(entrustDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'entrust/entrustList?entrustDel='+entrustDel+"&getName="+getName+"&getValue="+getValue+queryParam + urls,
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
		         checkbox:true,
	...
	function add(){
    	...
    	dlgCenter();
    	//点击保存按钮后跳转路径
    	url = "entrust/saveOrUpEntrust?" + urls;
	}
	...
	function setBaseInfoName(){
		...
		if(baseInfoId!="" && baseInfoName!="" && (role=="3" || role=="4")){
			$("#entrustUndertaker").textbox('setValue',baseInfoName);
			$("#entrustUndertakerId").val(baseInfoId);
			$("#entrustUndertaker").textbox("disable");
			$("#a_entrustUndertaker").hide();
		}
	}
	function edit(){
		// 获取操作对应的菜单ID
        ...
        if (row){
        	$('#fm').form('load', 'entrust/entrustView?path=editEntrust&entrustId='+row.entrustId + urls); 
        	$("#dlg").dialog({  
        		title: "咨政服务-其他委托-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	               /* minimizable: true,
	                maximizable: true,*/
	...
	function del(){
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].entrustId;  
                 } else {  
                     temID = selRow[i].entrustId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(entrustDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('entrust/deleteEntrust?entrustDel='+entrustDel + urls,{entrustId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('entrust/deleteEntrust?entrustDel=0' + urls,{entrustId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		var url = "ReportServer?reportlet=otherEntrust.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("其他委托");
		window.open(url);
    }
	// 上传材料
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="cs_other_entrust"
		$("#relevanceId").val(row.entrustId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.entrustId);
	}
	
	function uploadList(tableName,entrustId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+entrustId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=otherEntrust/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	//保存信息
    function saveUser(){
    	if($("#entrustUndertaker").prop('disabled')){
    		$("#entrustUndertaker").textbox("enable");
    	}
       $('#fm').form('submit',{
           url: url,
           onSubmit: function(){
