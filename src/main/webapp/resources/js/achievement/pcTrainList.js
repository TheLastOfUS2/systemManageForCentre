	...
	$(document).ready(function() { 
		...
		// 当手动填写培训人时,清空id
		$("input",$("#trainName").next("span")).keydown(function(){
			$("#trainNameId").val("");
		});
		//查询列表信息
		queryAction(0,'','');
	...
	//调用后台方法
	function queryAction(trainDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'train/trainList?trainDel='+trainDel+"&getName="+getName+"&getValue="+getValue+queryParam + urls,
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
		         checkbox:true,
	...
	function setBaseInfoName(){
		...
		if(baseInfoId!="" && baseInfoName!="" && (role=="3" || role=="4")){
			$("#trainName").textbox('setValue',baseInfoName);
			$("#trainNameId").val(baseInfoId);
			$("#trainName").textbox("disable");
			$("#a_trainName").hide();
		}
	}
	function edit(){
        ...
        if (row){
        	$('#fm').form('load', 'train/trainView?path=editTrain&trainId='+row.trainId + urls); 
        	$("#dlg").dialog({  
        		title: "人才培养-培训深造-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	              /*  minimizable: true,
	                maximizable: true,*/
	...
	function del(){
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].trainId;  
                 } else {  
                     temID = selRow[i].trainId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(trainDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('train/deleteTrain?trainDel='+trainDel + urls,{trainId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('train/deleteTrain?trainDel=0' + urls,{trainId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		var url = "ReportServer?reportlet=train.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("培训深造");
		window.open(url);
	}
	...
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="pc_train"
		$("#relevanceId").val(row.trainId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.trainId);
	}
	
	function uploadList(tableName,trainId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+trainId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=train/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	//保存信息
    function saveUser(){
    	if($("#trainName").prop('disabled')){
    		$("#trainName").textbox("enable");
    	}
       $('#fm').form('submit',{
           url: url,
           onSubmit: function(){
