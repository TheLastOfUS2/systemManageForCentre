	...
	$(document).ready(function() { 
		...
		// 当手动填写主讲人时,清空id
		$("input",$("#lectureSpeaker").next("span")).keydown(function(){
			$("#lectureSpeakerId").val("");
		});
	...
	//调用后台方法
	function queryAction(lectureDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'lecture/lectureList?lectureDel='+lectureDel+"&getName="+getName+"&getValue="+getValue+queryParam + urls,
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
		         checkbox:true,
	...
	function add(){
    	...
    	dlgCenter();
    	//点击保存按钮后跳转路径
    	url = "lecture/saveOrUpLecture?" + urls;
	}
	...
	function setBaseInfoName(){
		...
		if(baseInfoId!="" && baseInfoName!="" && (role=="3" || role=="4")){
			$("#lectureSpeaker").textbox('setValue',baseInfoName);
			$("#lectureSpeakerId").val(baseInfoId);
			$("#lectureSpeaker").textbox("disable");
			$("#a_lectureSpeaker").hide();
		}
	}
	...
	function edit(){
        ...
        if (row){
        	$('#fm').form('load', 'lecture/lectureView?path=editLecture&lectureId='+row.lectureId + urls); 
        	$("#dlg").dialog({  
        		title: "学术会议-报告讲座-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	               /* minimizable: true,
	                maximizable: true,*/
	...
	function del(){
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].lectureId;  
                 } else {  
                     temID = selRow[i].lectureId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(lectureDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('lecture/deleteLecture?lectureDel='+lectureDel + urls,{lectureId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('lecture/deleteLecture?lectureDel=0' + urls,{lectureId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		//获取搜索框中的内容
		var getValue= $('#searchboxId').searchbox('getValue');
		var url = "ReportServer?reportlet=lecture.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("报告讲座");
		window.open(url);
	}
	...
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="am_lecture"
		$("#relevanceId").val(row.lectureId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.lectureId);
	}
	
	function uploadList(tableName,lectureId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+lectureId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=lecture/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	//保存信息
    function saveUser(){
    	if($("#lectureSpeaker").prop('disabled')){
    		$("#lectureSpeaker").textbox("enable");
    	}
       $('#fm').form('submit',{
           url: url,
           onSubmit: function(){
