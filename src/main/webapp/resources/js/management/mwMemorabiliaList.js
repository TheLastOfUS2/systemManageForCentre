	...
	//调用后台方法
	function queryAction(memorabiliaDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'memorabilia/memorabiliaList?memorabiliaDel='+memorabiliaDel+"&getName="+getName+"&getValue="+getValue + urls,
			title:"基地大事件列表",
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
	...
	function add(){
    	...
    	dlgCenter();
    	//点击保存按钮后跳转路径
    	url = "memorabilia/saveOrUpMemorabilia?" + urls;
	}
	function edit(){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
        ...
        if (row){
        	$('#fm').form('load', 'memorabilia/memorabiliaView?path=editMemorabilia&memorabiliaId='+row.memorabiliaId + urls); 
        	$("#dlg").dialog({  
        		title: "管理工作-基地大事件-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	                /*minimizable: true,
	                maximizable: true,*/
					...
					onOpen:function(){
   					//将列表中的类型放到新增信息form中
					$('#memorabiliaId').val(row.memorabiliaId);
   					//加载对应通讯信息
   					pageDisplay();
   				},
        	});
	...
	function del(){
		...
		var memorabiliaDel=$("#memorabiliaDel").val();
		//返回选中多行
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].memorabiliaId;  
                 } else {  
                     temID = selRow[i].memorabiliaId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(memorabiliaDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('memorabilia/deleteMemorabilia?memorabiliaDel='+memorabiliaDel + urls,{memorabiliaId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('memorabilia/deleteMemorabilia?memorabiliaDel=0' + urls,{memorabiliaId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		//获取搜索框中的内容
		var getValue= $('#searchboxId').searchbox('getValue');
		var url = "ReportServer?reportlet=memorabilia.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("基地大事记");
		window.open(url);
	}
	// 上传材料
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="mw_memorabilia"
		$("#relevanceId").val(row.memorabiliaId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.memorabiliaId);
	}
	
	function uploadList(tableName,memorabiliaId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+memorabiliaId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=memorabilia/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	function pageDisplay(){
		...
		var mailList =$("#Staff").append("<table id='Staff1' class='StaffInfo'>" +
				...
				"<input type='text' id='memorabiliaTheme' name='memorabiliaTheme' class='easyui-textbox' style='width:448px'></td>" +
				"</tr>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>时间:</label></td>" +
				"<td width='180'>" +
				"<input type='text' id='memorabiliaTime' name='memorabiliaTime' class='easyui-datebox'></td>" +
				"</tr>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>简要内容:</label></td>" +
				"<td width='180' colspan='3'>" +
				"<input type='text' data-options='multiline:true' id='memorabiliaContent' name='memorabiliaContent'" +
				...
				"<td width='120' style='text-align:right;'><label class='in_label'>备注:</label></td>" +
	         	"<td width='180' colspan='3'>" +
	         	"<input type='text' data-options='multiline:true' id='memorabiliaRemarks' name='memorabiliaRemarks'" +
