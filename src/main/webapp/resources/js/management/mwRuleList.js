	...
	//调用后台方法
	function queryAction(ruleDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'rule/ruleList?ruleDel='+ruleDel+"&getName="+getName+"&getValue="+getValue + urls,
			title:"规章制度列表",
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
	...
	function edit(){
        ...
        if (row){
        	$('#fm').form('load', 'rule/ruleView?path=editRule&ruleId='+row.ruleId + urls); 
        	$("#dlg").dialog({  
        		title: "管理工作-规章制度-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	               /* minimizable: true,
	                maximizable: true,*/
	...
	function del(){
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].ruleId;  
                 } else {  
                     temID = selRow[i].ruleId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(ruleDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('rule/deleteRule?ruleDel='+ruleDel + urls,{ruleId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('rule/deleteRule?ruleDel=0' + urls,{ruleId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		//获取搜索框中的内容
		var getValue= $('#searchboxId').searchbox('getValue');
		var url = "ReportServer?reportlet=rule.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("规章制度");
		window.open(url);
	}
	// 上传材料
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="mw_rule"
		$("#relevanceId").val(row.ruleId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.ruleId);
	}
	
	function uploadList(tableName,ruleId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+ruleId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=rule/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	function pageDisplay(){
		...
		var mailList =$("#Staff").append("<table id='Staff1' class='StaffInfo'>" +
				"<tbody>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>制度名称:</label></td>" +
				"<td width='180' colspan='3'>" +
				"<input type='text' id='ruleTitle' name='ruleTitle' class='easyui-textbox' style='width:448px'></td>" +
				"</tr>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>颁布时间:</label></td>" +
				"<td width='180'>" +
				"<input type='text' id='ruleTime' name='ruleTime' class='easyui-datebox'></td>" +
				"</tr>"+
				"<tr>" +
				"<td width='120' style='text-align:right;'><label class='in_label'>简要内容:</label></td>" +
	         	"<td width='180' colspan='3'>" +
	         	"<input type='text' data-options='multiline:true' id='ruleContent' name='ruleContent'" +
				...
				"<td width='120' style='text-align:right;'><label class='in_label'>备注:</label></td>" +
	         	"<td width='180' colspan='3'>" +
	         	"<input type='text' data-options='multiline:true' id='ruleRemarks' name='ruleRemarks'" +
