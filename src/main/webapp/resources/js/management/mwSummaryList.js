	...
	//调用后台方法
	function queryAction(summaryDel,getName,getValue){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		
		//1.首先获取当前页号和每页显示条数
		$("#dg").datagrid({  
			url:'summary/summaryList?summaryDel='+summaryDel+"&getName="+getName+"&getValue="+getValue + urls,
			title:"基地总结列表",
			//加载列表数据
			 columns : [ [ {
		         field : 'ck',
                ...
                formatter : function(value,row,index){  
      				 if(value!=null && value!=''){
      				   if(value=='0'){return '年度总结'} else if(value=='1'){return '十二五规划'}
      				else if(value=='2'){return '十一五规划'}else if(value=='3'){return '十五规划'}
      				 } else{return '--'}
                   }
            }] ],
            /*datagrid绑定右键菜单*/
	...
	function add(){
    	...
    	dlgCenter();
    	//点击保存按钮后跳转路径
    	url = "summary/saveOrUpSummary?" + urls;
	}
	function edit(){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
        ...
        if (row){
        	$('#fm').form('load', 'summary/summaryView?path=editSummary&summaryId='+row.summaryId + urls); 
        	$("#dlg").dialog({  
        		title: "管理工作-基地总结-信息编辑",
					iconCls : "icon-edit",
					collapsible: true,
	               /* minimizable: true,
	                maximizable: true,*/
	...
	function del(){
        ...
        if (selRow!=""){
             ...
             for (i = 0; i < selRow.length;i++) {  
                 if (temID =="") {  
                     temID = selRow[i].summaryId;  
                 } else {  
                     temID = selRow[i].summaryId + "," + temID;  
                 }                 
             }
             var delTitle;
             if(summaryDel==1){
            ...
            $.messager.confirm('确认',delTitle,function(r){
                if (r){
                    $.post('summary/deleteSummary?summaryDel='+summaryDel + urls,{summaryId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function recovery(){
        ...
        if (selRow!=""){
            ...
            $.messager.confirm('确认','确定要恢复吗?',function(r){
                if (r){
                    $.post('summary/deleteSummary?summaryDel=0' + urls,{summaryId:temID},function(data){
                    	if(data.status == 1){
                            $('#dg').datagrid('reload');
                            $.messager.show({
                                title: '提示',
	...
	function exportExcel(){
		...
		var url = "ReportServer?reportlet=summary.cpt&" + getName + "=" + getValue + "&format=excel&extype=simple&__filename__=" + cjkEncode("基地总结");
		window.open(url);
	}
	function getValueFunction(name, value){
		//总结周期
        if(name=="summaryCycle"){
            if(value=="年度总结"){
               value=0; 
            }else if(value=="十二五规划"){
               value=1;
	...
	function upload(){
		...
		var row = $('#dg').datagrid('getSelected');
		//表名
		var tableName="mw_summary"
		$("#relevanceId").val(row.summaryId);
		$("#tableName").val(tableName);
		uploadList(tableName,row.summaryId);
	}
	
	function uploadList(tableName,summaryId){
		// 获取操作对应的菜单ID
		urls = getPowerMenuId();
		...
		// 首先根据id查询附件
		$.ajax({
			url:"common/fileList?relevanceId="+summaryId+"&tableName="+tableName + urls,
			type:"post",
	...
	function saveUploadFile(){
		...
		if(fileFullName!=""){
			$("#fmUpload").ajaxSubmit({
				type: "POST",
				url:"common/uploadFile?pathWay=summary/sr0&fileFullName="+fileFullName+"&tableName=bi_base_info&fileType="+fileType+"&relevanceId=1&fileContent="+fileContent + urls,
				dataType: "json",
			    success: function(data){
			    	if(data.status == 1){
			    		// 关闭附件信息弹框
	...
	function pageDisplay(){
		...
		var mailList =$("#Staff").append("<table id='Staff1' class='StaffInfo'>" +
				"<tbody>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>总结标题:</label></td>" +
				"<td width='180' colspan='3'>" +
				"<input type='text' id='summaryTitle' name='summaryTitle' class='easyui-textbox' style='width:448px'></td>" +
				"</tr>"+
				"<tr>"+
				"<td width='120' style='text-align:right;'><label class='in_label'>撰写时间:</label></td>" +
				"<td width='180'>" +
				"<input type='text' id='summaryTime' name='summaryTime' class='easyui-datebox'></td>" +
				"<td width='120' style='text-align:right;'><label class='in_label'>总结周期:</label></td>" +
				"<td width='180'>" +
				"<select id='summaryCycle' class='easyui-combobox' name='summaryCycle' panelMaxHeight='100px'>" +
	         	"<option value=''>请选择</option><option value='0'>年度总结</option><option value='1'>十二五规划</option>" +
	         	"<option value='2'>十一五规划</option><option value='3'>十五规划</option></td>" +
				"</tr>"+
				"<tr>" +
				"<td width='120' style='text-align:right;'><label class='in_label'>总结内容:</label></td>" +
	         	"<td width='180' colspan='3'>" +
	         	"<input type='text' data-options='multiline:true' id='summaryContent' name='summaryContent'" +
				...
				"<td width='120' style='text-align:right;'><label class='in_label'>备注:</label></td>" +
	         	"<td width='180' colspan='3'>" +
	         	"<input type='text' data-options='multiline:true' id='summaryRemarks' name='summaryRemarks'" +
